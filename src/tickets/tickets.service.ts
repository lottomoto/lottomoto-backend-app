...
export class TicketsService {
  ...
  private parseBoules(ligne: CreateTicketLigneDto): {
    boule1: number;
    boule2: number | null;
    boule3: number | null;
    prefix: number | null;
    ...
    if (ligne.type === LottoType.BORLETTE) {
      if (ligne.numero.length !== 2)
        throw new BadRequestException(
          `Borlette doit avoir 2 chiffres: ${ligne.numero}`,
        );
      return {
        boule1: parseInt(ligne.numero),
        boule2: null,
        boule3: null,
        prefix: null,
      };
    ...
    if (ligne.type === LottoType.MARIAGE) {
      ...
      if (num.length !== 4)
        throw new BadRequestException(
          `Mariage doit avoir 4 chiffres: ${ligne.numero}`,
        );
      return {
        boule1: parseInt(num.substring(0, 2)),
        boule2: parseInt(num.substring(2, 4)),
        boule3: null,
        prefix: null,
      };
    }
    if (ligne.type === LottoType.BLOTTO3) {
      if (ligne.numero.length !== 3)
        throw new BadRequestException(
          `Lotto 3 doit avoir 3 chiffres: ${ligne.numero}`,
        );
      return {
    ...
    if (ligne.type === LottoType.LOTTO4 || ligne.type === LottoType.BLOTTO4) {
      if (ligne.numero.length !== 4)
        throw new BadRequestException(
          `Lotto 4 doit avoir 4 chiffres: ${ligne.numero}`,
        );
      return {
    ...
    if (ligne.type === LottoType.LOTTO5 || ligne.type === LottoType.BLOTTO5) {
      if (ligne.numero.length !== 5)
        throw new BadRequestException(
          `Lotto 5 doit avoir 5 chiffres: ${ligne.numero}`,
        );
      return {
        prefix: parseInt(ligne.numero.substring(0, 1)),
        boule1: parseInt(ligne.numero.substring(1, 3)),
    ...
    if (ligne.type === LottoType.JACKPOT) {
      if (ligne.numero.length !== 6)
        throw new BadRequestException(
          `Jackpot doit avoir 6 chiffres: ${ligne.numero}`,
        );
      return {
        boule1: parseInt(ligne.numero.substring(0, 2)),
        boule2: parseInt(ligne.numero.substring(2, 4)),
  ...
  private async checkBouleRestrictions(
    ...
    const b1 = await this.bouleRepository.findOne({
      where: { numero: boule1 },
    });
    const b2 =
      boule2 !== null
        ? await this.bouleRepository.findOne({ where: { numero: boule2 } })
        : null;
    ...
    if (b1Blocked && b2Blocked) {
      throw new BadRequestException(
        `Les 2 boules ${String(boule1).padStart(2, '0')} et ${String(boule2).padStart(2, '0')} sont bloquées`,
      );
    } else if (b1Blocked && boule2 === null) {
      throw new BadRequestException(
        `La boule ${String(boule1).padStart(2, '0')} est bloquée`,
      );
    }
    ...
    const lim2 =
      boule2 !== null
        ? await this.limitationRepository.findOne({
            where: { bouleId: b2?.id, borlette: borletteName, tirage, date },
          })
        : null;

    const isSingleBoule =
      type === LottoType.BORLETTE || type === LottoType.BLOTTO3;
    const montantAajouter = isSingleBoule ? prix : prix / 2;

    if (lim1) {
      const totalVentes = await this.getTotalVentes(
        boule1,
        borletteId,
        tirage,
        date,
      ...
      if (totalVentes + montantAajouter > Number(lim1.montant)) {
        throw new BadRequestException(
          `Boule ${String(boule1).padStart(2, '0')} a atteint sa limite de ${lim1.montant} HTG (déjà ${totalVentes.toLocaleString()} HTG)`,
        );
      }
    }
    ...
    if (lim2 && boule2 !== null) {
      ...
      if (totalVentes + montantAajouter > Number(lim2.montant)) {
        throw new BadRequestException(
          `Boule ${String(boule2).padStart(2, '0')} a atteint sa limite de ${lim2.montant} HTG (déjà ${totalVentes.toLocaleString()} HTG)`,
        );
      }
    }
  }

  private async getTotalVentes(
    bouleNum: number,
    borletteId: string,
    tirage: string,
    date: string,
    ...
    const result = await this.ligneRepository
      ...
      .select(
        "COALESCE(SUM(CASE WHEN l.type IN ('borlette', 'blotto3') THEN l.prix ELSE l.prix / 2 END), 0)",
        'total',
      )
      .getRawOne();
  ...
  async create(dto: CreateTicketDto, vendeurUserId: string): Promise<any> {
    ...
    if (currentTime >= tirage.fermeture) {
      throw new BadRequestException(
        `Le tirage ${tirage.nom} est fermé (fermeture à ${tirage.fermeture})`,
      );
    }
    ...
    for (const l of parsedLignes) {
      await this.checkBouleRestrictions(
        l.boule1,
        l.boule2,
        dto.borletteId,
        dto.tirage,
  ...
  async remove(id: string): Promise<void> {
    ...
    if (ticket.status !== TicketStatus.EN_ATTENTE) {
      throw new BadRequestException(
        'Seuls les tickets en attente peuvent être supprimés',
      );
    }
  ...
  async payTicket(id: string): Promise<any> {
    ...
    if (ticket.status !== TicketStatus.GAGNE) {
      throw new BadRequestException(
        'Seuls les tickets gagnants peuvent être payés',
      );
    }
  ...
  async getVendeurStats(vendeurUserId: string): Promise<any> {
    ...
    let commissionRate = vendeur?.commission
      ? Number(vendeur.commission)
      : null;
  ...
  async getAdminStats(
    ...
    const paiementsTotal = payes.reduce(
      (s, t) => s + Number(t.gainTotal || 0),
      0,
    );
    ...
    const vendeurMap: Record<
      string,
      { nom: string; ventes: number; tickets: number }
    > = {};
    for (const t of tickets) {
      ...
      if (!vendeurMap[uid]) {
        vendeurMap[uid] = {
          nom: t.vendeurUser
            ? `${t.vendeurUser.firstname} ${t.vendeurUser.lastname}`
            : uid,
          ventes: 0,
          tickets: 0,
        };
    ...
    const chartMap: Record<
      string,
      { recettes: number; paiements: number; aPayer: number; benefice: number }
    > = {};
    for (const t of tickets) {
      const d = t.date;
      if (!chartMap[d])
        chartMap[d] = { recettes: 0, paiements: 0, aPayer: 0, benefice: 0 };
    ...
    for (const t of tickets) {
      ...
      revenueByTirageMap[d][t.tirage] =
        (revenueByTirageMap[d][t.tirage] || 0) + Number(t.total);
    }
    ...
    // Boules bloquées
    const boulesBloquees = await this.bouleRepository.count({
      where: { status: BouleStatus.BLOQUEE },
    });
    ...
    const activeTirage = allTirages.find(
      (t) => currentTime >= t.ouverture && currentTime < t.fermeture,
    );
